...
async function generateImage(
  ...
  try {
    ...
    // OpenAI SDK 返回的是 ImagesResponse 类型，直接访问 data 数组
    ...
    try {
      ...
      for (let j = 0; j < result.imagePrompts.length; j++) {
        ...
        if (imgResult && "url" in imgResult) {
          ...
          print("gray", "  URL: " + imgResult.url.substring(0, 80) + "...\n");
        } else {
          print("red", "  FAILED: " + (imgResult?.error || "Unknown error") + "\n");
        }
