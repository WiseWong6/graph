...
| **Upload 节点修复** | ✅ 完成 | 2026-01-18 | state.wechat + FormData 文件名 |
...
**Upload Images 节点** (`12_upload.node.ts`) ✅ (2026-01-18 修复):
- [x] 使用 `state.wechat` 配置（修复：不再读取 `WECHAT_APP_ID`）
- [x] Node.js FormData 修复（使用 `File` 对象指定文件名）
- [x] 并行上传（可配置并发）
- [x] 返回 CDN URL
- [x] 单点测试通过 (`scripts/test-upload-only.ts`)
...
| **阶段2** | 05_draft | Research+RAG → 文章（强制标题） | ✅ 完成 |
| | 07_rewrite | 智性叙事转换 | ✅ 完成 |
| **阶段3** | 08_confirm | 图片数量确认 | ✅ 完成 |
| | 09_humanize | 去 AI 味效果 | ✅ 完成 |
| | 10_prompts | 5种风格提示词生成 | ✅ 完成 |
| | 11_images | Ark API 生图 | ✅ 完成 |
| | 12_upload | 微信 CDN 上传 | ✅ 完成 |
...
**本次更新**:
1. **Upload 节点修复** ✅ (2026-01-18)
   - **问题1**: 使用 `getWechatConfig()` 读取 `WECHAT_APP_ID`（但实际是 `WECHAT_APP_ID_1/2`）
     - 修复: 直接使用 `state.wechat`
     - 删除 `getWechatConfig()` 函数
   - **问题2**: Node.js FormData 缺少文件名（微信 API 返回 `invalid file type hint`）
     - 修复: 使用 `File` 对象指定 `filename="image.png"` 和 `type="image/png"`
   - 单点测试通过 (`scripts/test-upload-only.ts`)
   - 成功上传到微信 CDN 并返回 URL

2. **10_prompts 节点 Reasoner 泄露修复** ✅ (2026-01-18)
   - 切换模型: `deepseek-reasoner` → `deepseek-chat`
   - 超时优化: 120s → 60s（chat 模型更快）
   - 消除思考过程泄露到控制台的问题
   - JSON 输出更清晰可靠

3. **Gate C (select_title) 增强** ✅
   - 添加自定义标题输入功能（含可选备注）
   - 添加重新生成标题选项
...
- [x] Upload 节点 state.wechat 使用问题 (2026-01-18)
- [x] Upload 节点 FormData 文件名问题 (2026-01-18)
...
│   │   │   ├── 10_prompts.node.ts        ✅
│   │   │   ├── 11_images.node.ts         ✅
│   │   │   ├── 12_upload.node.ts         ✅ (修复: state.wechat + File)
│   │   │   ├── 13_html.node.ts           ✅
│   │   │   └── 14_draftbox.node.ts       ✅
│   ├── rag/                              ✅
│   ├── adapters/                         ✅
...
**重构说明**:
- **2026-01-18 (本次更新)**:
  - Upload 节点修复: 使用 `state.wechat` + File 对象
